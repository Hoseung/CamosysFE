...
class PostProcessor:
    ...
    def run(self, label_array):
        ...
        while True:
            ...
            keypoints_2d = label_array['body_keypoints2d'][0]
            keypoints_2d_conf = label_array['body_keypoints2d_conf'][0]
            key3d = label_array['body_keypoints3d'][0]
            flmk_x = label_array['face_landmarks_x'][0]
            flmk_y = label_array['face_landmarks_y'][0]
            ...
            if box is not None and box[0] > self.area_lmin and box[3] < self.area_rmax:
                ...
                # full body visible?
                if keypoints_2d_conf[0] > 90 and keypoints_2d_conf[11] > 90 and keypoints_2d_conf[12] > 90:
                    ...
                    if dist:
                        ...
                        if cnt > self.cnt_initial:
                            ...
                            # check for head size
                            face_width = np.mean(flmk_x[14:] - flmk_x[:3]) / dist
                            face_height = np.mean(flmk_y[7:10] - flmk_y[32:35]) / dist
                            initial_guess_face_h.append(face_height)
                            initial_guess_face_w.append(face_width)
                            
                        elif cnt == self.cnt_initial: 
                            running_avg = np.percentile(initial_guess, 90)
                            face_width_final = np.percentile(initial_guess_face_w, 90)
                            face_height_final = np.percentile(initial_guess_face_h, 90)
                ...
                if cnt % 100 == 1 and running_avg is not None:
                    if running_avg > 40 and running_avg < 1.20:
                        label_array['passenger'][0] = 1
                    elif running_avg < 1.58:
