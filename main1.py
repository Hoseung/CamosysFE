...
class MainWindow(QWidget):
    ...
    def update_view(self):
        if not self.client.frame_queue.empty() and not self.client.label_data_queue.empty():
            ...
            # face_bounding_box
            face_bounding_box_x = np.round(label_data["face_bounding_box_x"][0] * frame_width_resize_ratio).astype(int)
            face_bounding_box_y = np.round(label_data["face_bounding_box_y"][0] * frame_height_resize_ratio).astype(int)
            #
            # color = (46, 234, 255)
            #
...
def main():
    ...
    client = Client(server_ip='169.254.244.73')  # 127.0.0.1'
    client.setup_socket()
